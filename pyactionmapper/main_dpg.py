...
dpg.configure_viewport(0, width=920, height=300, max_width=920, decorated=True, resizable=False)

from theme import global_theme

# with dpg.font_registry():
#     default_font = dpg.add_font("fonts/27_RussellSquare.ttf", 12)
#     header_font = dpg.add_font("fonts/27_RussellSquare.ttf", 14)
...
class mapper():
    ...
    xml_dir = Path("{}/xml".format(src_dir))
    ...
    def switch_profile(self, a_profile):
        ...
        print(f"selected_profile.name: {self.selected_profile.name}")
        self.selected_profile.load_actionmaps(mapper.dtd_actionmap)
        dpg.configure_item("profile_label", show=True, label=f"Profile name = {self.selected_profile.name}")
        # self.profile_name.set(f"Profile name = {self.selected_profile.name}")
        self.u_map = self.selected_profile.actionmaps
        print(f"u_map: {self.u_map}")
        ...
        for s_idx, section_name in enumerate(self.tab_names):
            print(f"tab names: {self.tab_names}")
            ...
            print(f"u_vers: {u_vers}")
            d_vers = section["version"]
            print(f"d_vers: {d_vers}")
            if u_vers != d_vers:
            ...
            for a_idx, action in enumerate(section["action"]):
                ...
                print(f"action_keys: {self.action_keys}")
                print(f"action[\"name\"]: {a['name']}")
                try:
    ...
    def setup_display(self):
        ...
        self.tab_names = self.default_map.section_names()           # list of tab names
        print(f"tab_names: {self.tab_names}")
        # not sold on calling this method configure... but that is what the user would be doing.
        with dpg.window(label=mapper.app_name) as self.main_window:
            with dpg.menu_bar():
                with dpg.menu(label="File"):
                    dpg.add_menu_item(label="Restore Backup", callback=self.restore_backup)
                    dpg.add_menu_item(label="Save", callback=self.selected_profile.save_actionmaps)
                    dpg.add_separator()
                    dpg.add_menu_item(label="Exit", callback=self.exit_window)
                dpg.add_menu_item(label="Help", callback=donothing)
        ...
        # self.profile_name.set(f"Profile name = {self.selected_profile.name}")
        # self.profile_label = ttk.Label(self.main_window,
        #                                textvariable=self.profile_name)  # text=self.selected_profile.name)
        # self.profile_label.pack(expand=1, fill="both")
        #
            self.profile_label = dpg.add_button(parent=self.main_window, tag="profile_label")
            dpg.add_button(parent=self.main_window, show=True)
            print(dpg.get_value(self.profile_label))
        ...
        #     for a_idx, action in enumerate(section["action"]):
        #         try:
        #             self.action_labels[section_name][action["name"]] = ttk.Label(self.section_tabs[s_idx],
    ...
    def exit_window(self, _sender, _data):
        dpg.stop_dearpygui()
